...
def main():
    ...
    even_better_draw_circles(rg.Point(-200, 0), 10, 20, 'green', 5)
    even_better_draw_circles(rg.Point(100,300), 10, 50, 'pink', 2)
    even_better_draw_circles(rg.Point(-300, -300), 15, 20, 'blue', 10)
    even_better_draw_circles(rg.Point(0, -250), 30, 10, 'orange', 15)
...
def better_draw_circles(point, radius_step):
    """
    ...
    for k in range(1, 11):  # k becomes 1, 2, 3, ... 10
        ...
        # Go DOWN 15 pixels, ending up pointing east again
        turtle.right(90)
        turtle.forward(radius_step)
        turtle.left(90)

        turtle.pen_down()
        turtle.draw_circle(radius_step * k)  # Radius 15, 30, 45, 60, ...
...
def even_better_draw_circles(point, radius_step, num_of_circles, color, thickness):
